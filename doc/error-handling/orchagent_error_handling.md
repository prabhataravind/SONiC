...
* Read aforementioned APP_DB tables once for unconsumed entries.
* Wait 30 seconds.
...
query_orchagent will act as the notification producer on ORCHAGENTQUERY channel and be notification consumer on the ORCHAGENTQUERYREPLY channel while switchorch in orchagent will act as the notification producer on ORCHAGENTQUERYREPLY channel and act as the consumer on ORCHAGENTQUERY channel.

    // Send pending entries query via "ORCHAGENTQUERY" notification channel
    swss::NotificationProducer pendingQuery(&db, "ORCHAGENTQUERY");
    //  Listen for the reply on "ORCHAGENTQUERYREPLY" channel
    swss::NotificationConsumer pendingQueryReply(&db, "ORCHAGENTQUERYREPLY");
...
3. At the end of Orchagent's main while loop after doTask() for all orchs have been called, orchagent will call getPendingEntries() function to retrieve the entries that are present in m_toSync map only if switchorch's m_getPendingEntries is set to True
